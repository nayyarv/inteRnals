...
3. The R Language
  ...
  - Lazy Evaluation and basic metaprogramming
4. Building a Pipe
5. Conclusion
...
- Environments have a hierarchy, each function from `pryr` is defined in that environemnt, and the environments of those functions are linked to the parent one one
...
- Radford Neal - [Patches to improve R speed by 25% and make more consistent](https://radfordneal.wordpress.com/2010/08/15/two-surpising-things-about-r/)
  - Has since released pqR which is more performant than GNU R, but diverges on syntax
- Christian Robert - [R’s not-so-brilliant language and/or interpreter](https://xianblog.wordpress.com/2010/09/08/julien-on-r-shortcomings/). Loves the libraries, dislikes the language
...
- The R language for all it's flaws, has a solid base in LISP/Scheme and gives developer's the keys to the kingdom. 
- Python's success in DS is almost certainly related to R's popularity and problems
  - Common ancestor in LISP and numeric library (numpy) available in late 2008.
  - Very similar languages, and much better CS fundamentals
  - Would have likely still been popular given the CS 
- R still lacks generic and efficient data structures to make all purpose programming easy.
  - Many parts of the base langauge are still unoptimised
  
  
## R and I

- I still use R, especially when generating reports for less data savvy people.
- I don't see R as a programming language, more of a tool
- I still use R often (most of my work with the greens), but as a user not a developer.
  - I rarely use more than one .R or .Rmd file and rarely reuse R code
  - Anything long term goes into a different language. `dplyr` syntax doesn't play nicely in functions.
- R has a very steep learning curve. Most will never reach Advanced R. 
- Heavily influenced me to head down more functional programming
